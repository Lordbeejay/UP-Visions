...
label act3_loop:
    ...
    if _action == "walk":
        call walk_to_node(_node, nodes=act3_nodes, player_zoom=6)
        call expression _node.target_label
...
label act5_map:
    ...
    $ player_facing = "up"
    $ act5_nodes = [
        MapNode("kuya_rico", 2500, 2000, "act5_npc_kuya_rico", tooltip="Kuya Rico", icon_image="kuya_rico.png", locked=False, icon_zoom=0.25),
    ]
...
label act5_cl3_map:
    ...
    $ player_facing = "up"
    $ act5_nodes = [
        MapNode("prof_lena", 2500, 1800, "act5_npc_prof_lena", tooltip="Prof. Lena", icon_image="prof_lena.png", locked=False, icon_zoom=0.25),
    ]
    $ current_task_text = "Talk to Prof. Lena in CL3"

label act5_cl3_loop:
    call screen map_screen("maps/CL3.png", act5_nodes, current_task_text, 1.0, player_zoom=6)
    $ _action, _node = _return

    if _action == "walk":
        call walk_to_node(_node, nodes=act5_nodes, player_zoom=6)
        call expression _node.target_label
...
label act5_ow_cas2_map:
    ...
    $ player_facing = "up"
    $ act5_nodes = [
        MapNode("ate_grace",     2000, 2000, "act5_npc_ate_grace",     tooltip="Ate Grace", icon_image="ate_grace.png",     locked=False, icon_zoom=0.25),
        MapNode("classmate_dan", 3000, 2500, "act5_npc_classmate_dan", tooltip="Dan",       icon_image="classmate_dan.png", locked=True,  icon_zoom=0.25),
        MapNode("first_class",   2500, 1200, "act5_first_class",       tooltip="Enter CL3", icon_image="npcs/Arrow.png",    locked=True),
    ]
    $ current_task_text = "Talk to Ate Grace"

label act5_ow_cas2_loop:
    call screen map_screen("ace/OW_CAS.png", act5_nodes, current_task_text, 1.0, player_zoom=6)
    $ _action, _node = _return
...
## ============================================================================
## ACT 6 MAP — Student Support Services
## Story: Find Dan → he's unwell → HSU → GCSU → Scholarship Service
## Single entry node; the rest flows as one narrative chain.
## ============================================================================
...
label act6_map:
    ...
    $ player_facing = "up"
    $ act6_nodes = [
        MapNode("dan", 1800, 2400, "act6_npc_dan", tooltip="Dan", icon_image="classmate_dan.png", locked=False, icon_zoom=0.25),
    ]
...
label act6_complete:
    scene black
    call screen act_transition("ACT 6 COMPLETE", "You've navigated the support services\nthat hold UP Visayas together.", "complete")
...
## Story: Find Jaden outside the library → chains through all scenes.
## Single entry node; the rest flows as one narrative chain.
## ============================================================================
...
label act7_map:
    ...
    $ player_facing = "up"
    $ act7_nodes = [
        MapNode("jaden", 2200, 2400, "act7_npc_jaden", tooltip="Jaden", icon_image="jaden.png", locked=False, icon_zoom=0.25),
    ]
    $ current_task_text = "Find Jaden near the library"
...
label act7_complete:
    scene black
    call screen act_transition("ACT 7 COMPLETE", "Library, TLRC, study group.\nYou finally know how to study at UP.", "complete")
...
## Story: Find Jaden → one chain through canteen, dorm, Prof. Reyes, oval.
## Single entry node; the rest flows as one narrative chain.
## ============================================================================
...
label act8_map:
    ...
    $ player_facing = "up"
    $ act8_nodes = [
        MapNode("jaden", 2100, 2400, "act8_npc_jaden", tooltip="Jaden", icon_image="jaden.png", locked=False, icon_zoom=0.25),
    ]
    $ current_task_text = "Find Jaden — it's the end of the week"

label act8_loop:
    call screen map_screen("ui/CAS_Overworld(F).png", act8_nodes, current_task_text, 1.0)
    $ _action, _node = _return
...
label act8_complete:
    scene black
    call screen act_transition("ACT 8 COMPLETE", "End of Week One.\nYou know where to go now.", "complete")
    call screen act_transition("UP VISAYAS", "Honor and Excellence.", "complete")
